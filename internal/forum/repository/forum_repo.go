...
func (rep *Repository) GetThreads(tx *pgx.Tx, f *models.Forum, desc, limit, since string) ([]models.Thread, error) {
	...
	var sqlStr string
	if desc == "true" {
		sqlStr = "SELECT t.id, t.title, t.message, t.created, t.slug, t.usr, f.slug, t.votes FROM thread t " +
	...
	for rows.Next() {
		...
		slug := sql.NullString{}
		th := models.Thread{}
		err = rows.Scan(&th.Id, &th.Title, &th.Message, &created, &slug, &th.Author, &th.Forum, &th.Votes)
		if err != nil {
			return nil, err
		}
		if slug.Valid {
